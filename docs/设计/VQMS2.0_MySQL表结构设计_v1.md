...
10. **单电压等级在运**（Leo 2026-09-02 确认）：运行时同一时点只有 220kV 或 500kV **其中一档**在运/在考，不会两档并存。台账按档分别登记（0/1 与 2），运行时数据对上哪档就统计哪档，无需设计双档并行交互；判定/报表按母线分组天然隔离。
...
1. ~~busbar 2（500kV）是否纳入~~ **已拍板（2026-09-02 Leo）：运行时 220kV 与 500kV 只采用其中一档、不会并存**——台账按档分别登记（含 busbar 2），运行时数据对上哪档统计哪档，单档考核
